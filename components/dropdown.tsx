...
import { View, TouchableOpacity, StyleSheet, LayoutRectangle, Pressable } from 'react-native';
      ...
      {menuVisible && dropdownLayout && (
        <>
          ...
          <View
            ...
            {options?.map(item => (
              <TouchableOpacity key={item.id} style={styles.menuItem} onPress={() => handleSelect(item)}>
                <Text>{item.text}</Text>
              </TouchableOpacity>
            ))}
          </View>
