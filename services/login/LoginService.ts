...
import asyncStorage from '../../store/AuthStorage';
import { IAuthenResultModel, ILoginInfo, ILoginModel, IUserLoginDto } from './LoginDto';
...
class LoginService {
  ...
  GetLoginResult = async (input: ILoginModel, tennatId: number) => {
    ...
    const response = await fetch(`${REACT_NATIVE_API_URL}api/TokenAuth/GetLoginResult`, requestOptions);
    const jsonData = await response.json();
    return jsonData.result;
    try {
    } catch (error) {}
